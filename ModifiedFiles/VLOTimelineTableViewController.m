...
@implementation VLOTimelineTableViewController {
    CGFloat _kWelcomeMessageCellHeight;
}
...
- (void)viewDidLoad
{
    ...
    _kWelcomeMessageCellHeight = 150.0f;
    ...
    if (!_isViewMode) {
        [[VLOWatchHandler sharedWatchHandler] initLocationManager];
    }
...
- (void)addLogToDataSource:(VLOLog *)log
{
    ...
    [self validTimeInfoToLog:log]; // 예외 처리
    
    
    NSInteger logOrder = _logs.count;
    
    if (_addToBelowPivotLog && [_logs containsObject:_addToBelowPivotLog]) {
        logOrder = [_logs indexOfObject:_addToBelowPivotLog] + 1;
        log.date = _addToBelowPivotLog.date;
        log.timezone = _addToBelowPivotLog.timezone;
    }
    ...
    for (NSInteger i = logOrder-1; i >= 0; i --) {
        ...
        if (log.previousCellId.length > 0 && [log.previousCellId isEqualToString:logAtIndex.timelineId])
        {
            logOrder = i + 1;
            previousLog = logAtIndex;
            break;
        }
        else if (log.previousCellId.length > 0 || logAtIndex.type == VLOLogTypeDay)
        {
            continue;
        }
        // 기본 로직
        ...
        if ([log.date compare:logAtIndex.date] <= 0)
        {
            ...
            if (logAtIndex.type == VLOLogTypeDay) {
                if (i == 0) {
                    ...
                    previousLog = ((VLOLog *)[_logs objectAtIndex:i-1]);
                }
            }
            break;
        }
    ...
    if (!previousLog) {
        ...
        log.previousCellId = previousLog.timelineId;
    }
    ...
    if (log.timelineId == nil) {
        if (logOrder > 0 && [log.date compare:((VLOLog *)[_logs objectAtIndex:logOrder-1]).date] < 0) {
...
- (void)validTimeInfoToLog:(VLOLog *)log
{
    if (log.date == nil) {
        if ([_logs count] == 0) {
            ...
            log.date = ((VLOLog *)[_logs lastObject]).date;
        }
    }
    if (log.timezone == nil) {
...
- (void)updateLog:(VLOLog *)log isSetToBelow:(VLOLog *)previous
{
    ...
    [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellMenu action:VLOActionChangeOrder label:nil andValue:@(labs(beforeIndex - afterIndex))];
}
...
- (void)checkAndSetEmptyContent
{
    ...
    if (_logs.count == 0) {
        ...
        */
        ...
        UIImageView *emptyContentBackgroundImageView = [[UIImageView alloc] init];
        emptyContentBackgroundImageView.image = [UIImage imageNamed:@"timeline_empty_pattern_1080"];
        emptyContentBackgroundImageView.contentMode = UIViewContentModeScaleAspectFill;

        self.tableView.backgroundView = emptyContentBackgroundImageView;

        CGRect headerFrame = CGRectMake(0, 0, self.tableView.frame.size.width, 170.0f);
        self.tableView.tableHeaderView = [[VLOTimelineTableHeaderView alloc] initWithFrame:headerFrame];
...
{
    NSInteger numOfRows = _logs.count;
    return numOfRows;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    VLOLog *log = [_logs objectAtIndex:row];
...
- (void)setLikeToCell:(VLOTimelineCell *)cell
{
    ...
    [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellMenu action:VLOActionLikeCell label:[VLOLog typeStringWithType:log.type] andValue:nil];
}
...
//                    break;
            ...
            [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellShare action:VLOActionShareCellFB label:[VLOLog typeStringWithType:sharedCellType] andValue:@(cellTarget)];
        ...
        if (sharedCellType == VLOLogTypePhoto || sharedCellType == VLOLogTypeRoute) {
            ...
            VLOActionSheetItem *instagramShareItem = [[VLOActionSheetItem alloc] initWithTitle:NSLocalizedString(@"timeline_share_menu_instagram",) color:instagramColor font:itemFont handler:^{
                ...
                if (canOpenInstagram) {
                    ...
                    [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellShare action:VLOActionShareCellInsta label:[VLOLog typeStringWithType:sharedCellType] andValue:@(cellTarget)];
    ...
    BOOL isSharableCell = (sharedCellType != VLOLogTypeText && sharedCellType != VLOLogTypeTitle);
    if ( shareSection && isSharableCell /* && isPublic */ ) {
        ...
        */
        ...
        if (isMyLog) {
            VLOActionSheetItem *editItem = [[VLOActionSheetItem alloc] initWithTitle:[NSString stringWithFormat:NSLocalizedString(@"actionSheet_editCell", ), type]
                ...
                [self editCell];
                                                                                 [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellMenu action:VLOActionEdit label:[VLOLog typeStringWithType:sharedCellType] andValue:nil];
            }];
            ...
            [[VLOActionSheetItem alloc] initWithTitle:removeItemTitle
                 ...
                 [alert show];
                                                  [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellMenu action:VLOActionDelete label:[VLOLog typeStringWithType:sharedCellType] andValue:nil];
             }];
...
- (void)editCell
{
    ...
    if (!log) {
        [self.delegate timelineTableViewController:self didEditingWasStarted:NO];
        return;
    }
    ...
    if (log.type == VLOLogTypeText) {
        ...
        if (![log.user.userId isEqualToString:[VLOLocalStorage getCurrentUser].userId]) {
            ...
            [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryCellMenu action:VLOActionAddPhoto label:userCount andValue:@(photoCount)];
        }
    }
    else if (log.type == VLOLogTypeMap) {
...
- (void)photoLogEditor:(VLOPhotoLogEditorViewController *)editor didAddPhoto:(VLOPhotoLog *)log
{
    ...
    [VLOAnalyticsManager reportGAEventWithCategory:VLOCategoryAddPhoto action:VLOActionDone label:userCount andValue:@(addedPhotoCount)];
